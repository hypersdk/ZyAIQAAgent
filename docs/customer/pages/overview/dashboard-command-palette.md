...
⌘K / Ctrl-K spotlight to jump to any action card by name.
...
- UI: Mission Control → **Overview** → **Command palette** (scroll the Actions grid, or search with ⌘K / Ctrl-K)

## What you can do

1. Open `/dashboard` (sign in at `/login` when `DASHBOARD_PASSWORD` is set).
2. Fill the card fields for **Command palette**, then start the action and watch the live job panel (✓/✗ chips, Stop, download log).
