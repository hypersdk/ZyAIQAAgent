...
Session login gate for Mission Control when DASHBOARD_PASSWORD is configured.
...
- Surface: `/login`
- UI: Mission Control → **Overview** → **Login** (scroll the Actions grid, or search with ⌘K / Ctrl-K)

## What you can do

1. Open `/dashboard` (sign in at `/login` when `DASHBOARD_PASSWORD` is set).
2. Fill the card fields for **Login**, then start the action and watch the live job panel (✓/✗ chips, Stop, download log).
